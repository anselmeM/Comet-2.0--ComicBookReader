...
export default function ReaderPage() {
  ...
  return (
    <div
      className={`relative h-screen w-full bg-black overflow-hidden ${isFullscreen ? 'cursor-none' : ''}`}
    >
      {/* The Reading Engine */}
      ...
      <div
        className={`transition-opacity duration-500 z-50 ${isMenuVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
      >
      ...
      {isFullscreen && !isMenuVisible && (
        <div className="absolute top-4 right-4 text-white/20 text-[10px] uppercase font-bold pointer-events-none">
          <div className="flex flex-col items-end">
            ...
            <span>
              Press <kbd className="bg-neutral-700 px-1.5 py-0.5 rounded text-xs">ESC</kbd> or{' '}
              <kbd className="bg-neutral-700 px-1.5 py-0.5 rounded text-xs">F</kbd> to exit
            </span>
          </div>
        </div>
      )}
