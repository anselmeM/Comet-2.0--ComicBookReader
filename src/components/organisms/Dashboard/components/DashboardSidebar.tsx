...
export function DashboardSidebar({ isOpen, activeView, onNavClick }: DashboardSidebarProps) {
  ...
  return (
    <motion.aside
      ...
      className="bg-white border-r border-neutral-50 flex flex-col py-14 shrink-0 overflow-y-auto no-scrollbar"
    >
      <motion.div
        initial={false}
        ...
        {navItems.map((item) => (
          ...
          >
            <item.icon
              size={24}
              strokeWidth={2.5}
              className={`shrink-0 ${activeView === item.id ? 'text-white' : ''}`}
            />
            <motion.span
              initial={false}
              animate={{
