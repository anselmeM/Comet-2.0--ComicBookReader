    ...
    >
      ...
      {results.comics.length > 0 && (
        <section className="space-y-6">
          <h3 className="text-sm font-black uppercase tracking-widest text-neutral-400 border-l-4 border-blue-500 pl-4">Comics ({results.comics.length})</h3>
      ...
      {results.collections.length > 0 && (
        <section className="space-y-6">
          <h3 className="text-sm font-black uppercase tracking-widest text-neutral-400 border-l-4 border-purple-500 pl-4">Collections ({results.collections.length})</h3>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {results.collections.map(col => (
              <button 
                key={col.id}
                onClick={() => { setActiveView('collections'); onClearSearch(); }}
              ...
              >
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-neutral-400 font-bold uppercase tracking-widest">{col.items?.length || 0} items</p>
                  </div>
                </div>
                <ChevronDown className="-rotate-90 text-neutral-200 group-hover:text-purple-500 transition-colors" size={24} />
              </button>
            ))}
          </div>
        </section>
      ...
      {results.series.length > 0 && (
        <section className="space-y-6">
          <h3 className="text-sm font-black uppercase tracking-widest text-neutral-400 border-l-4 border-orange-500 pl-4">Series ({results.series.length})</h3>
      ...
      {results.comics.length === 0 && results.collections.length === 0 && results.series.length === 0 && (
