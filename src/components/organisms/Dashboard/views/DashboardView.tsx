    ...
    <div className="space-y-12 animate-in fade-in duration-500 pb-20">
          ...
          {featuredComic && (
            <div className="absolute top-8 right-8 z-20 flex gap-2">
              ...
              >
                <Heart
                  size={24}
                  className={`${isFeaturedFav ? 'text-red-500 fill-red-500' : 'text-white'}`}
                />
              </button>
            </div>
          )}
        ...
        <section className="lg:col-span-4 bg-[#0F172A] rounded-[2.5rem] p-10 text-white flex flex-col justify-between shadow-2xl relative overflow-hidden group">
          {continueComic && (
            ...
            >
              <Heart
                size={18}
                className={`${isContinueFav ? 'text-red-500 fill-red-500' : 'text-slate-400'}`}
              />
            </button>
          )}
        ...
        <div className="flex flex-wrap gap-8">
          {favouriteHeroes.slice(0, 4).map((hero) => (
            <div key={hero.id} className="group flex flex-col items-center gap-4 cursor-pointer">
              <div
                className={`relative w-24 h-24 rounded-full p-[3px] overflow-hidden transition-all duration-500 group-hover:-translate-y-2 group-hover:scale-105 border-2 border-transparent bg-neutral-100/50 ${HERO_GLOWS[hero.name] || 'group-hover:shadow-xl'}`}
              >
                <Image
                  src={hero.image}
                  alt={hero.name}
              ...
              <span className="text-sm font-black text-neutral-400 group-hover:text-blue-500 transition-colors uppercase tracking-widest text-[10px]">
                {hero.name}
              </span>
            </div>
          ))}
          ...
          >
            <div className="w-24 h-24 rounded-full bg-neutral-100 flex items-center justify-center border-2 border-dashed border-neutral-200 transition-all group-hover:border-blue-500 group-hover:bg-blue-50 group-hover:text-blue-500">
              <Users
                size={32}
                strokeWidth={1.5}
                className="text-neutral-300 group-hover:text-blue-500"
              />
            </div>
            <span className="text-[10px] font-black text-neutral-300 uppercase tracking-widest group-hover:text-blue-500 transition-colors">
              View All
            </span>
          </div>
        </div>
      ...
      <section className="space-y-8">
        <div className="flex items-center justify-between">
          ...
          <button className="text-xs font-black uppercase tracking-widest text-blue-500 hover:underline">
            View All
          </button>
        </div>
          ...
          {topRatedComics.map((comic) => (
            <div key={comic.id} className="group flex flex-col gap-4">
              ...
              <div className="flex flex-col gap-1">
                <h4 className="text-sm font-black text-blue-600 group-hover:underline decoration-2 underline-offset-2 cursor-pointer line-clamp-1">
                  {comic.title}
                </h4>
