...
export const HistoryView = ({
  ...
  const recentlyRead = comics
    .filter(c => (c.progress?.lastPage ?? 0) > 0);
    ...
    <div className="space-y-12 animate-in fade-in duration-500 pb-20">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-6">
          ...
          <div>
            <h2 className="text-4xl font-black text-neutral-900 tracking-tighter italic">Reading History</h2>
            <p className="text-sm font-bold text-neutral-400 uppercase tracking-widest mt-1">Pick up where you left off</p>
          </div>
        </div>
      </div>
      ...
      {recentlyRead.length > 0 ? (
              ...
              <div key={comic.id} className="bg-white p-6 rounded-[2.5rem] border border-neutral-100 shadow-sm flex gap-6 group hover:shadow-xl transition-all">
                ...
                <div className="flex-1 flex flex-col justify-between py-2">
                  <div className="space-y-2">
                    <span className="text-[10px] font-black uppercase text-blue-500 tracking-widest">Last Read</span>
                    <h4 className="text-xl font-black text-neutral-900 tracking-tighter leading-tight line-clamp-2">{comic.title}</h4>
                    <p className="text-xs font-bold text-neutral-400 uppercase tracking-widest">{comic.author || 'Unknown Artist'}</p>
                  </div>
                  
                  <div className="space-y-4">
                    <div className="space-y-2">
                      <div className="flex justify-between items-end">
                        <span className="text-sm font-black text-neutral-900 italic">{progress}%</span>
        ...
        <div className="text-center py-40 bg-white/50 rounded-[3rem] border-2 border-dashed border-neutral-100">
          ...
          <h4 className="text-2xl font-black text-neutral-400 uppercase tracking-tighter italic">No history yet</h4>
