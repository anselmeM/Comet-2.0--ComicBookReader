    ...
    <div className="space-y-10 animate-in fade-in duration-500 pb-20 text-neutral-800">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-neutral-100 pb-8">
        <div className="flex items-center gap-6">
          <button
            ...
            className="p-4 bg-white border border-neutral-100 rounded-2xl hover:bg-neutral-50 transition-all text-neutral-400 hover:text-comet-accent shadow-sm cursor-pointer"
          >
            <ChevronLeft size={24} />
          </button>
      ...
      <div className="flex gap-2 bg-neutral-100 p-1.5 rounded-2.5xl max-w-md">
        <button
          ...
          className={`flex-1 py-3 rounded-2xl font-black text-xs uppercase tracking-wider transition-all cursor-pointer ${
            activeTab === 'stats'
              ? 'bg-white text-comet-accent shadow-sm'
      ...
      {isHistoryLoading ? (
        <div className="py-24 flex flex-col items-center gap-4">
          <Loader2 size={40} className="animate-spin text-comet-accent" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
              ...
              <div className="lg:col-span-8 bg-white p-4 sm:p-6 lg:p-8 rounded-[2.5rem] border border-neutral-100 shadow-sm space-y-6">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 bg-comet-accent/10 rounded-xl flex items-center justify-center text-comet-accent">
                    <Calendar size={20} />
                  </div>
                      ...
                      {heatmapWeeks.map((week, colIdx) => (
                        <g key={colIdx} transform={`translate(${colIdx * 12}, 0)`}>
                          {week.map((cell, rowIdx) => {
                            ...
                            if (cell.intensity === 1)
                              cellColorClass = 'fill-comet-accent/25 hover:fill-comet-accent/40';
                            else if (cell.intensity === 2)
                              cellColorClass = 'fill-comet-accent/55 hover:fill-comet-accent/70';
                            else if (cell.intensity === 3)
                              cellColorClass = 'fill-comet-accent/85 hover:fill-comet-accent';
                            else if (cell.intensity === 4)
                              cellColorClass = 'fill-comet-accent hover:fill-comet-accent-hover';
                ...
                <div className="flex items-center justify-end gap-2 text-[10px] font-bold text-neutral-400 uppercase tracking-widest pr-4 select-none">
                  ...
                  <div className="w-3.5 h-3.5 bg-comet-accent/15 rounded-md"></div>

                  <div className="w-3.5 h-3.5 bg-comet-accent/50 rounded-md"></div>

                  <div className="w-3.5 h-3.5 bg-comet-accent/100 rounded-md"></div>

                  <div className="w-3.5 h-3.5 bg-comet-accent-hover rounded-md"></div>
                        ...
                        return (
                          <g key={idx} className="group cursor-pointer">
                            <rect
                              ...
                              width={20}
                              height={barHeight}
                              className="fill-comet-accent/85 group-hover:fill-comet-accent transition-colors rx-md"
                              rx={4}
                            />
                            ...
                            <text
                              ...
                              y={y - 8}
                              textAnchor="middle"
                              className="text-[9px] font-extrabold fill-comet-accent opacity-0 group-hover:opacity-100 transition-opacity"
                            >
                              {week.count}
                            </text>
              ...
              <div className="space-y-4 max-h-[500px] overflow-y-auto pr-2 scrollbar-thin">
                {historyData?.sessions && historyData.sessions.length > 0 ? (
                  historyData.sessions.map((session) => (
                    <div
                      key={session.id}
                      className="bg-neutral-50 p-5 rounded-2xl border border-neutral-100 flex items-center justify-between group hover:bg-white hover:border-comet-accent/40 transition-all shadow-sm"
                    >
                      <div className="flex items-center gap-5 min-w-0">
                        ...
                        <div className="min-w-0">
                          <p className="text-neutral-900 font-medium text-sm leading-relaxed">
                            Read{' '}
                            <span className="font-black text-comet-accent">
            ...
            <div className="space-y-6">
              <div className="flex flex-col md:flex-row gap-4 items-center justify-between">
                <h3 className="text-xl font-black text-neutral-900 tracking-tight italic flex items-center gap-3">
                  <Award size={20} className="text-comet-accent" />
                ...
                {recentlyReadComics.length > 0 && (
                  <div className="relative w-full md:max-w-md flex items-center">
                    ...
                    <input
                      ...
                      className="w-full bg-white border border-neutral-100 rounded-2xl pl-12 pr-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-comet-accent/20 focus:border-comet-accent transition-all placeholder:text-neutral-400 placeholder:font-bold"
                    />
                  </div>
                )}
              </div>
                        ...
                        <div className="flex-1 flex flex-col justify-between py-1 min-w-0">
                          <div className="space-y-1">
                            <div className="flex justify-between items-start gap-4">
                              <span className="text-[9px] font-black uppercase text-comet-accent tracking-widest block truncate">
                          ...
                          <div className="space-y-3 pt-1">
                            ...
                            <div className="h-1.5 bg-neutral-100 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-comet-accent/100 rounded-full transition-all duration-500"
