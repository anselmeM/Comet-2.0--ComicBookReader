    ...
    <div className="max-w-4xl mx-auto w-full space-y-8">
      <div className="flex items-center justify-between">
        <h3 className="text-xl font-black text-comet-text tracking-tight flex items-center gap-3 uppercase tracking-widest">
          <Zap size={20} className="text-comet-accent" />
          Live Community Feed
        </h3>

        {isLoadingFeed && <Loader2 size={16} className="text-comet-accent animate-spin" />}
      </div>
        ...
        {feed && feed.length > 0 ? (
          feed.map((activity) => (
            <motion.div
              ...
              className="bg-comet-surface p-4 sm:p-6 rounded-[2.5rem] border border-comet-border shadow-sm flex flex-col gap-4 group hover:border-comet-accent/40 transition-all"
            >
              <div className="flex items-center gap-6 w-full">
                <div className="relative shrink-0">
                  ...
                  <div
                    className={`absolute -bottom-1 -right-1 w-6 h-6 rounded-lg flex items-center justify-center border-2 border-white shadow-sm ${
                      activity.type === 'FINISHED' ? 'bg-green-500' : 'bg-comet-accent/100'
                    }`}
                  >
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-comet-text font-medium">
                    ...
                    <span className="text-comet-accent font-bold italic tracking-tight">
                      {activity.comicTitle}
                    </span>
                  </p>
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold transition-all shadow-sm border cursor-pointer ${
                          reacted
                            ? 'bg-comet-accent/10 border-comet-accent/20 text-comet-accent scale-105'
