      ...
      <div className="bg-comet-surface p-4 sm:p-6 lg:p-8 rounded-[2.5rem] border border-comet-border shadow-sm space-y-8 h-full">
        <div>
          <h3 className="text-xl font-black text-comet-text tracking-tight flex items-center gap-2 mb-2">
            <Search className="text-comet-accent" size={20} />
            Search Readers
          </h3>
        ...
        <div className="relative">
          <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-comet-muted" size={20} />
          ...
          <input
            ...
            className="w-full pl-14 pr-6 py-4 bg-comet-surface-2 border-2 border-transparent focus:border-comet-accent focus:bg-comet-surface rounded-2xl outline-none transition-all font-bold text-base"
          />

          {isSearching && (
            <div className="absolute right-6 top-1/2 -translate-y-1/2">
              <Loader2 size={20} className="text-comet-accent animate-spin" />
            </div>
          )}
        </div>
          ...
          {searchQuery.length >= 2 ? (
            searchResults && searchResults.length > 0 ? (
              searchResults.map((user) => (
                <div
                  key={user.id}
                  className="bg-comet-surface-2 p-5 rounded-2xl border border-comet-border flex items-center justify-between group hover:bg-comet-surface hover:border-comet-accent/40 transition-all"
                >
                  ...
                  <div className="flex shrink-0">
                    {user.status === 'FRIEND' ? (
                      ...
                      <div className="text-comet-accent p-2" title="Request Sent">
                        <Loader2 size={20} className="animate-spin" />
                      </div>
                    ) : user.status === 'REQUEST_RECEIVED' ? (
                      <button
                        onClick={() => handleAcceptRequest(user.requestId!)}
                        className="bg-comet-accent text-white p-2 rounded-lg hover:scale-105 active:scale-95 transition-all shadow-md cursor-pointer"
            ...
            <div className="text-center py-10 bg-comet-surface-2 rounded-2xl border border-dashed border-comet-border">
              ...
              <p className="text-xs font-bold text-comet-muted">Enter at least 2 characters</p>
            </div>
          )}
        </div>
      </div>

      <div className="bg-gradient-to-br from-comet-accent to-comet-accent-hover p-10 rounded-[2.5rem] shadow-xl text-white space-y-8 flex flex-col justify-center relative overflow-hidden h-full">
        <div className="absolute top-0 right-0 -mr-20 -mt-20 w-64 h-64 bg-comet-surface/10 rounded-full blur-3xl pointer-events-none" />

        <div className="absolute bottom-0 left-0 -ml-20 -mb-20 w-64 h-64 bg-white/10 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="relative">
          ...
          <p className="text-white/85 font-medium leading-relaxed max-w-sm">
            Invite your fellow comic book enthusiasts to join Comet. We&apos;ll automatically make
            you friends once they sign up!
          </p>
        </div>

        <form onSubmit={handleInviteEmail} className="relative space-y-4">
          <div className="relative">
            <Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-white/65" size={20} />
            ...
            <input
              ...
              className="w-full pl-14 pr-6 py-5 bg-comet-surface/10 border-2 border-white/10 focus:border-white focus:bg-comet-surface/20 rounded-3xl outline-none transition-all font-bold text-white placeholder:text-white/75 backdrop-blur-sm"
            />
          </div>
          ...
          <button
            ...
            className="w-full bg-comet-surface text-comet-accent py-5 rounded-3xl font-black uppercase tracking-[0.2em] text-xs flex items-center justify-center gap-3 hover:scale-[1.02] active:scale-95 transition-all shadow-2xl disabled:opacity-50 cursor-pointer"
          >
            {isInviting ? (
