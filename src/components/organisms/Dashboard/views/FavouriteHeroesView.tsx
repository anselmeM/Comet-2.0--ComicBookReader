    ...
    <div className="grid grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-10">
      {favouriteHeroes.map((hero) => (
        <div key={hero.id} className="group flex flex-col items-center gap-6 cursor-pointer">
          <div
            className={`relative w-32 h-32 md:w-40 md:h-40 rounded-full p-2 overflow-hidden transition-all duration-500 group-hover:-translate-y-3 group-hover:scale-105 border-2 border-transparent bg-neutral-100/50 group-hover:shadow-[0_20px_50px_-15px_rgba(0,0,0,0.3)]`}
          >
            {hero.image ? (
              <Image
                src={hero.image}
              ...
              <div
                className={`w-full h-full flex items-center justify-center font-black text-4xl md:text-6xl ${heroAvatarClass(hero.name)}`}
              >
