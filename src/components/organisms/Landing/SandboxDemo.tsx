...
export default function SandboxDemo() {
  ...
  const [sandboxDetectedPanels, setSandboxDetectedPanels] = useState<
    ...
    <section className="mt-48 w-full max-w-7xl mx-auto text-left relative z-20">
      <div className="bg-neutral-950/40 backdrop-blur-xl border-3 border-neutral-850 rounded-[2.5rem] p-8 md:p-12 shadow-2xl flex flex-col lg:flex-row gap-12 items-stretch">
        ...
        <div className="flex-1 flex flex-col justify-between space-y-8">
          <div>
            <div className="inline-flex items-center gap-2 text-comet-lime font-display font-black uppercase tracking-widest text-xs mb-4">
              <Zap size={12} className="text-comet-lime animate-pulse" />
              TRY OFFLINE PARSING
            </div>
            ...
            <p className="text-neutral-400 text-sm max-w-lg leading-relaxed font-medium">
              Delegating compression routes to Web Workers prevents main-thread lag. Drag a local
              `.cbz` or `.cbr` file here to verify decompression throughput on your device.
            </p>
          </div>
          ...
          <div
            ...
            className={`border-3 border-dashed rounded-2xl p-8 text-center cursor-pointer transition-all duration-300 relative overflow-hidden group flex flex-col items-center justify-center min-h-[220px] ${
              sandboxIsDragging
                ? 'border-comet-orange bg-comet-orange/10 scale-[1.01] shadow-[4px_4px_0px_0px_rgba(255,90,0,0.15)]'
                : 'border-neutral-855 bg-comet-ink/40 hover:border-comet-orange/50 hover:bg-comet-orange/5'
            }`}
          >
            <input
            ...
            {sandboxIsParsing ? (
              <div className="space-y-4 w-full px-4" aria-live="polite">
                <Loader2 className="w-10 h-10 text-comet-orange animate-spin mx-auto" />
                ...
                {sandboxProgress && (
                  <div className="w-full bg-neutral-900 h-2 rounded-full overflow-hidden border border-white/5">
                    <motion.div
                      className="h-full bg-comet-orange rounded-full shadow-[0_0_10px_rgba(255,90,0,0.4)]"
                      initial={{ width: 0 }}
                      animate={{
              ...
              <>
                <UploadCloud
                  className={`w-10 h-10 mb-4 transition-colors ${sandboxIsDragging ? 'text-comet-orange' : 'text-neutral-500 group-hover:text-comet-orange'}`}
                />
          ...
          {sandboxPages.length > 0 && sandboxMetrics ? (
            <div className="h-full flex flex-col justify-between space-y-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-comet-ink p-3 rounded-xl border border-neutral-850 text-center">
                  <span className="text-[9px] font-display font-black text-neutral-500 uppercase tracking-widest block">
                    Throughput
                  </span>

                  <span className="text-xl font-heading font-black text-comet-lime italic block mt-1">
                    {sandboxMetrics.throughput} p/s
                  </span>
                </div>

                <div className="bg-comet-ink p-3 rounded-xl border border-neutral-855 text-center">
                  <span className="text-[9px] font-display font-black text-neutral-500 uppercase tracking-widest block">
                    Decompress Time
                  </span>
                ...
                <div className="bg-comet-ink p-3 rounded-xl border border-neutral-855 text-center col-span-2 sm:col-span-1">
                  <span className="text-[9px] font-display font-black text-neutral-500 uppercase tracking-widest block">
                    Deduplication Hash
                  </span>
              ...
              <div className="flex-1 relative bg-comet-ink border-2 border-neutral-855 rounded-xl overflow-hidden aspect-[4/3] max-h-[300px] flex items-center justify-center shadow-inner group/viewer">
                {/* The Page Image */}
                          ...
                          {sandboxDetectedPanels[sandboxCurrentPage].map((panel, idx) => {
                            ...
                            return (
                              <motion.div
                                ...
                                className="absolute border-2 border-comet-red bg-comet-red/10 rounded shadow-[0_0_8px_rgba(239,68,68,0.3)]"
                                style={{
                              ...
                              >
                                <span className="absolute -top-4 left-0 bg-comet-red text-white text-[8px] font-heading font-black px-1.5 py-0.5 rounded-t shadow-md">
                                  {idx + 1}
                                </span>
                              </motion.div>
                            );
                ...
                {sandboxIsDetecting && (
                  <div className="absolute inset-0 bg-black/70 flex items-center justify-center gap-2">
                    <Loader2 className="w-5 h-5 text-comet-orange animate-spin" />
                ...
                <div className="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between pointer-events-none opacity-0 group-hover/viewer:opacity-100 transition-opacity">
                  <button
                    ...
                    className="p-3 bg-neutral-950/95 hover:bg-comet-ink-2 border-2 border-neutral-855 text-white rounded-xl transition-all pointer-events-auto disabled:opacity-30 disabled:pointer-events-none active:scale-90"
                  >
                    <ChevronLeft size={16} />
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[10px] font-heading font-black uppercase tracking-widest transition-all ${
                      sandboxShowPanels
                        ? 'bg-comet-orange text-white border-2 border-black shadow-[3px_3px_0px_0px_#000] active:translate-x-[1px] active:translate-y-[1px]'
