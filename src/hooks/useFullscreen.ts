...
export function useFullscreen({ 
  ...
  // Request fullscreen
  const requestFullscreen = useCallback(async (element: HTMLElement): Promise<void> => {
    if (!isSupported) {
...
export function useDocumentFullscreen() {
  return useFullscreen({
    elementRef: { current: typeof document !== 'undefined' ? document.body : null } as React.RefObject<HTMLElement>,
    onEnter: () => {},
    onExit: () => {},
  });
}
